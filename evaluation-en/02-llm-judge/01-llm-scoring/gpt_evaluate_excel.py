...
By default, the script reads ``benchmark_questions.xlsx`` from its own directory
and writes ``benchmark_llm_scores.xlsx``. Existing output is resumed automatically.
"""
...
REQUIRED_HEADERS = ("Question", "Answer 1", "Answer 2", "Gold Standard")
...
class DimensionEvaluation(BaseModel):
    score: Score
    justification: str = Field(description="A concise justification in English.")
...
- Write all justifications, issues, the overall assessment, and priority
  improvements in concise English.
...
RESULT_HEADERS = {
    "answer1": {
        "correctness": "Answer 1-Correctness",
        "usefulness": "Answer 1-Usefulness",
        "safety": "Answer 1-Safety",
        "detail": "Answer 1-Evaluation Details",
    },
    "answer2": {
        "correctness": "Answer 2-Correctness",
        "usefulness": "Answer 2-Usefulness",
        "safety": "Answer 2-Safety",
        "detail": "Answer 2-Evaluation Details",
    },
}
ANSWER_COLUMNS = {"answer1": "Answer 1", "answer2": "Answer 2"}
METADATA_HEADERS = ("Scoring Model", "Scoring Time")
...
def parse_args() -> argparse.Namespace:
    ...
    default_input = script_dir / "benchmark_questions.xlsx"

    parser = argparse.ArgumentParser(
        description=(
            "Use a GPT model to score answer1 and answer2 against Gold Standard "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output workbook (default: <input stem>_llm_scores.xlsx)",
    )
    parser.add_argument(
        "--model",
    ...
    if args.output is None:
        args.output = args.input.with_name(f"{args.input.stem}_llm_scores.xlsx")
...
def build_tasks(workbook, sheet_names: list[str], overwrite: bool) -> tuple[list[EvaluationTask], dict[str, dict[str, int]]]:
    ...
    for sheet_name in sheet_names:
        ...
        for row in range(2, worksheet.max_row + 1):
            question = normalized_text(worksheet.cell(row, columns["Question"]).value)
            gold = normalized_text(worksheet.cell(row, columns["Gold Standard"]).value)
            if not question and not gold:
                continue
            if not question or not gold:
                print(
                    f"Skip {sheet_name} row {row}: User issuesor Gold Standard is blank",
                    file=sys.stderr,
                )
                continue

            for answer_name in ("answer1", "answer2"):
                if not overwrite and answer_is_complete(
                    worksheet, row, columns, answer_name
                ):
                    continue
                candidate = normalized_text(
                    worksheet.cell(row, columns[ANSWER_COLUMNS[answer_name]]).value
                )
                if not candidate:
                    print(
...
def write_result(
    ...
    worksheet.cell(task.row, columns["Scoring Model"], model)
    worksheet.cell(
        task.row,
        columns["Scoring Time"],
