...
class Py2NeoGraphRAG:
    ...
    def __init__(
        ...
        if not api_key:
            raise ValueError("GraphRAG 需要提供 ZhipuAI API Key")
    ...
    def initialize(self) -> None:
        if not self.triples_path or not self.triples_path.exists():
            logger.warning("未找到知识图谱三元组文件，跳过构建: %s", self.triples_path)
            return
        ...
        try:
            ...
            logger.info("知识图谱构建完成，实体数: %s", len(self.st))
        except Exception as exc:  # pylint: disable=broad-except
            logger.error("知识图谱构建失败: %s", exc)
            self._graph_built = False

    def create_graph(self) -> None:
        logger.info("开始构建 Neo4j 知识图谱")
        ...
        for _, row in data.iterrows():
            ...
            try:
                ...
                logger.debug("创建关系失败: %s", exc)

        self.st = list(created_nodes)

    def create_embeddings(self) -> None:
        if not self.st:
            logger.warning("未找到任何实体，跳过向量创建")
            return
    ...
    def query_related_entities(self, entity_name: str) -> List[Dict[str, str]]:
        #双向 Cypher 查询（ outgoing + incoming ），全面获取与目标实体相关的所有实体及关系，返回结构化的三元组列表
        query_out = (
    ...
    def get_all_relation_types(self) -> List[str]:#获取知识图谱中所有不重复的关系类型的方法
    ...
    def _build_prompt(self, question: str, facts: Sequence[str]) -> str:
        ...
        return (
            f"请基于以下知识图谱事实回答用户的问题，并用中文给出基知识图谱的明确的答复：\n"
            f"知识图谱事实：\n{facts_text}\n"
            f"用户问题：{question}"
        )
    ...
    def _ensure_initialized(self) -> Optional[Py2NeoGraphRAG]:
        ...
        with self._init_lock:
            ...
            try:
                ...
                if not api_key:
                    raise ValueError("GRAPH_RAG_CONFIG 中缺少 ZHIPU_API_KEY")
